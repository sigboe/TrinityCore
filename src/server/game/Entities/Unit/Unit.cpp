...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    else
    {
        ...
        if (duel_hasEnded)
        {
            ...
            // Needed to prevent extra swings and onhit spells from landing on duelist (but this also blocks non-dueling player spells from landing on duelist)
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, uint32 stack /*= 1*/) const
{
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto /*= nullptr*/, SpellSchoolMask damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    if (spellProto)
    {
        ...
        {
            if ((aurEff->GetMiscValue() & spellProto->GetSchoolMask()) && aurEff->GetCasterGUID() == attacker->GetGUID() && aurEff->IsAffectedOnSpell(spellProto))
                return true;
            return false;
        });
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    if (Player* player = ToPlayer())
    {
        ...
        if (guardian)
            guardian->SetUnitFlag(UNIT_FLAG_STUNNED);
...
void Unit::Dismount()
{
    ...
    if (Player* player = ToPlayer())
    {
        ...
        else if (Guardian* pGuardian = player->GetGuardianPet())
        {
            if (pGuardian->HasUnitFlag(UNIT_FLAG_STUNNED) && !pGuardian->HasUnitState(UNIT_STATE_STUNNED))
                pGuardian->RemoveUnitFlag(UNIT_FLAG_STUNNED);
        }
        else
...
void Unit::PatchValuesUpdate(ByteBuffer& valuesUpdateBuf, BuildValuesCachePosPointers& posPointers, Player* target)
{
    ...
    if (posPointers.UnitFieldFactionTemplatePos >= 0)
    {
        ...
        {
            valuesUpdateBuf.put(posPointers.UnitFieldFactionTemplatePos, m_uint32Values[UNIT_FIELD_FACTIONTEMPLATE]);
        }
    }
}
